...
impl SqliteAssistantRepository {
    ...
    pub async fn execute_query(&self, query_expression: &str) -> RepositoryResult<String> {
        ...
        let answer = self.to_json_string(&rows).await?;
    ...
    async fn to_json_string(&self, rows: &Vec<SqliteRow>) -> RepositoryResult<String> {
        if rows.is_empty() {
            return Ok("[]".to_string());
        }
        ...
        for row in rows {
            ...
            for col in columns {
                ...
                map.insert(col.name(), val_str);
            }

            result.push(json!(map));
        }
